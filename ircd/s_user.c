...
int register_user(struct Client *cptr, struct Client *sptr)
{
  ...
  if (MyUser(sptr))
  {
    ...
    client_set_privs(sptr, NULL, 0, __LINE__);
		...
		char *parv[], int allow_modes)
{
  ...
  if (IsRegistered(sptr)) {
    if (!FlagHas(&setflags, FLAG_OPER) && IsOper(sptr)) {
      /* user now oper */
      ++UserStats.opers;
      client_set_privs(sptr, NULL, 0, __LINE__); /* may set propagate privilege */
    }
    ...
    if ((FlagHas(&setflags, FLAG_OPER) || FlagHas(&setflags, FLAG_LOCOP))
      ...
      client_set_privs(sptr, NULL, 0, __LINE__); /* will clear propagate privilege */
    }
